...
async function runTest() {
    ...
    try {
        ...
        if (!userRes.ok) throw new Error(`Create User failed: ${userRes.statusText}`);
        ...
        if (!formRes.ok) throw new Error(`Create Form failed: ${formRes.statusText}`);
        ...
        if (!submissionRes.ok) throw new Error(`Create Submission failed: ${submissionRes.statusText}`);
        ...
        if (!usersListRes.ok) {
            ...
            const found = usersList.find((u: any) => u.id === user.id);
            if (found) console.log("✅ Created user confirmed in list.");
        }
        ...
        if (!subListRes.ok) throw new Error("Fetch submissions failed");
